...
const COLS = 9;
const MAX_TILE = 48;
function calcTileSize() { return Math.min(MAX_TILE, Math.floor(window.innerWidth / ROWS)); }
let TILE_SIZE = calcTileSize();
let OUTER_BOARD_SIZE = TILE_SIZE * ROWS;
...
export default function BoardScreen() {
  ...
  useEffect(() => {
    const handler = () => {
      ...
      OUTER_BOARD_SIZE = TILE_SIZE * ROWS;
      _forceBoard(n => n + 1);
    };
    ...
    <div style={{
      ...
      {player.rebirthReadySwamp && player.rebirthReadyDCC && (
        <div style={{
          ...
          background: "linear-gradient(90deg, #001A2A 0%, #002A3A 50%, #001A2A 100%)",
