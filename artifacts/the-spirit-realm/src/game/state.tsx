...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  function rollDice() {
    ...
    if (tile.kind === "boss") {
      ...
      pendingAction = { type: "shop", items: genShopItems(p0.bossKills) };
  ...
  function calcDamageTaken(rawDmg: number, dtype: DamageType, blockCount: number, eq: Player["equipped"]): number {
    ...
    if (blockCount === 0) {
      ...
      const passiveReduction = Math.min(0.08, totalDef / (totalDef + 80));
      return Math.max(1, Math.round(rawDmg * (1 - passiveReduction)));
    }
    const baseReduction = blockCount >= 2 ? 0.50 : 0.25;
  ...
  function jwcEndRound() {
    ...
    if (!next.finished) {
      ...
      const newSp    = SP_PER_ROUND + reserved;
      const newEnemyActions = next.enemies.map(e =>
        e.hp > 0 ? decideEnemyAction(e) : ("def" as const)
      );
      logs.push(`── SP: ${newSp}${reserved > 0 ? ` (${SP_PER_ROUND}+${reserved} rsrv)` : ""} ──`);
  ...
  function forgeItem(which: "weapon" | "armor"): boolean {
    ...
    setPlayerState((q) => {
      ...
      const level = Math.round((range.min + range.max) / 2);
      return {
        ...q,
        metals: q.metals - 3,
        inventory: [
  ...
  function recordDCCWin(xpGain: number, moneyGain: number): { leveled: boolean; newLevel: number; rewardCard: Card } {
    let leveled = false;
    let newLevel = playerRef.current.dccLevel;
    const rewardCard = pickDCCRewardCard(playerRef.current.dccLevel);
    setPlayerState((p) => {
      const totalXp      = p.dccXp + xpGain;
      const xpPerLevel   = 100;
      const levelsGained = Math.floor(totalXp / xpPerLevel);
      if (levelsGained > 0) {
        leveled  = true;
        newLevel = p.dccLevel + levelsGained;
  ...
  function debugMoveToTile(kind: TileKind) {
    ...
    for (let offset = 1; offset <= len; offset++) {
      ...
      if (BOARD[idx].kind === kind) {
        ...
        if (kind === "shop") {
          setPendingTileAction({ type: "shop", items: genShopItems(p.bossKills, p.rebirthCount) });
        } else if (kind === "forge") {
  ...
  return (
    <GameContext.Provider value={{
      ...
      equipItem, unequipSlot, meltItem, sellItem, forgeItem,
