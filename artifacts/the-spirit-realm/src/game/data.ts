...
export function genShopItems(bossKills: number, rebirthCount = 0): Item[] {
  const biome = getBiome(bossKills);
  const range = getEnemyLevelRange(bossKills, false);
...
export const STARTING_DECK: Card[] = [
  ...
  { id: "c3",  name: "Archer",         cost: 2, power: 5,  text: "+3 if rightmost lane" },
  { id: "c4",  name: "Archer",         cost: 2, power: 5,  text: "+3 if rightmost lane" },
