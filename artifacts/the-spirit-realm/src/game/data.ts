...
export function rollEncounter(bossKills: number, elite = false): Enemy[] {
  ...
  if (elite) {
    ...
    const hp    = Math.round(60 + level * 8 + Math.floor(Math.random() * 15));
    return [{
      ...
      loot: {
        money:   Math.round(30 + level * 14 + Math.floor(Math.random() * 20) + (biome === "swamp" ? 25 : 0)),
  ...
  for (let i = 0; i < count; i++) {
    ...
    result.push({
      ...
      loot: {
        money:   Math.round((8 + level * 7 + Math.floor(Math.random() * 10) + (biome === "swamp" ? 25 : 0)) / count),
...
export function rollBoss(bossKills: number): Enemy[] {
  ...
  return [{
    ...
    loot: {
      money:   500 + level * 80 + 100,
      gems:    10 + level,
      metals:  10 + Math.floor(level / 2),
...
export function genArmor(level = 1): Armor {
  ...
  return {
    ...
    name:    variant.name,
    ...stats,
    bonusHP:  Math.max(1, Math.round((3 + level * 1.5) * (Math.random() * 0.4 + 0.8))),
    level,
  };
}
...
export const STARTING_DECK: Card[] = [
  ...
  { id: "c3",  name: "Archer",         cost: 1, power: 2,  text: "Vanguard: +4 if outnumber" },
  { id: "c4",  name: "Archer",         cost: 1, power: 2,  text: "Vanguard: +4 if outnumber" },
