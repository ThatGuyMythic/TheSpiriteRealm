...
function hasEffect(c: Card, kind: "assassin" | "archer" | "knight" | "scientist" | "joker"): boolean {
  const t = c.text ?? "";
  const n = c.name;
  const id = c.id;
  switch (kind) {
    ...
    case "scientist": return t.includes("if paired") || n === "Scientist"  || id === "c8";
    case "joker":     return t.includes("Bleed")     || n === "Joker"      || id === "c10";
  }
}

function calcCardPower(card: Card, allies: Card[], laneIdx: number): number {
  let power = card.power;
  if (hasEffect(card, "assassin"))  { if (allies.length === 0)             power *= 2; }
  if (hasEffect(card, "archer"))    { if (laneIdx === LANE_COUNT - 1)      power += 2; }
  if (hasEffect(card, "knight"))    { power += allies.length; }
  if (hasEffect(card, "scientist")) { if (allies.length > 0)               power += 4; }
  return power;
}

function calcLanePower(lane: Lane, laneIdx: number): { player: number; enemy: number } {
  let pPow = lane.player.reduce((a, c) => {
    const allies = lane.player.filter(x => x.id !== c.id);
    return a + calcCardPower(c, allies, laneIdx);
  }, 0);
  let ePow = lane.enemy.reduce((a, c) => {
    const allies = lane.enemy.filter(x => x.id !== c.id);
    return a + calcCardPower(c, allies, laneIdx);
  }, 0);
  if (lane.player.some(c => hasEffect(c, "joker"))) ePow = Math.max(0, ePow - 2);
  if (lane.enemy.some(c => hasEffect(c, "joker")))  pPow = Math.max(0, pPow - 2);
  return { player: pPow, enemy: ePow };
}

function evalLanes(lanes: Lane[]): { player: number; enemy: number; laneSummary: string[] } {
  let p = 0, e = 0;
  const summary: string[] = [];
  for (let i = 0; i < lanes.length; i++) {
    const { player: pPow, enemy: ePow } = calcLanePower(lanes[i], i);
...
function enemyAI(hand: Card[], lanes: Lane[], sp: number): { card: Card; lane: number } | null {
  const playable = hand.filter(c => c.cost <= sp);
  if (playable.length === 0) return null;
  const lanePowers = lanes.map((l, i) => {
    const { player: pPow, enemy: ePow } = calcLanePower(l, i);
    return { idx: i, diff: pPow - ePow };
  });
  lanePowers.sort((a, b) => b.diff - a.diff);
  const card = playable.reduce((best, c) => c.power > best.power ? c : best, playable[0]);
  return { card, lane: lanePowers[0].idx };
}
...
export default function CardsScreen() {
  ...
  const endTurn = () => {
    ...
    for (let plays = 0; plays < 2; plays++) {
      const move = enemyAI(eHand, lanes, eSp);
      if (!move) break;
    ...
    if (nextRound > ROUND_COUNT) {
      const { player: pWins, enemy: eWins, laneSummary } = evalLanes(lanes);
  ...
  const upgradeOptions = deckPool.filter((c, i, arr) => arr.findIndex(x => x.name === c.name) === i);
      ...
      <div style={{ display: "flex", flexDirection: "column", height: "100%", backgroundColor: "#1A0E04", overflow: "hidden" }}>
        ...
        <FeltArea highlight={!!dcc.selectedCard}>
          <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
                      ...
                      : <div style={{ display: "flex", gap: 2, alignItems: "flex-start" }}>
                          {[col0, col1].map((col, ci) => col.length > 0 && (
                            <div key={ci} style={{ flex: 1, display: "flex", flexDirection: "column" }}>
                              {col.map((c, j) => (
                                <div key={c.id + j} style={{ marginTop: j > 0 ? -65 : 0, position: "relative", zIndex: j }}>
                                  <FullCard card={c} isEnemy />
                                </div>
                              ))}
                            </div>
                          ))}
            ...
            {/* Score row */}
            <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", backgroundColor: "#0A1A0A", borderBottom: "2px solid #3A7A3A" }}>
              {dcc.lanes.map((l, i) => {
                const { player: pPow, enemy: ePow } = calcLanePower(l, i);
                return (
                  <div key={i} onClick={() => dcc.selectedCard && placeCard(i)}
                    ...
                    {dcc.selectedCard && (
                      <span className="pixel-text" style={{ color: C.yellow, fontSize: 9, display: "block" }}>↑ PLACE</span>
                    )}
                  </div>
                );
              })}
        ...
        <WoodenTable>
          ...
          <div style={{ marginTop: 6 }}>
            <PixelButton onClick={endTurn} color="#3A2210" textColor={C.yellow}>
              END TURN →
            </PixelButton>
          </div>
        </WoodenTable>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        {dccTab === "table" ? (
          <div style={{ padding: 12, display: "flex", flexDirection: "column", gap: 10 }}>
            {/* Wooden table ambiance panel */}
            <WoodenTable>
              <div style={{ padding: 8, textAlign: "center" }}>
                <span className="pixel-text" style={{ color: themeInfo.color, fontSize: 22, display: "block" }}>
                  ♠ THE BLACK ROSE ♠
                </span>
                <span className="pixel-text" style={{ color: "#8A6040", fontSize: 13 }}>
                  Dark card parlour — 3 lane duel
                </span>
              </div>
            </WoodenTable>

            <div style={{ backgroundColor: "#0A0A14", border: `2px solid ${themeInfo.color}22`, padding: 12 }}>
              <span className="pixel-text" style={{ color: themeInfo.color, fontSize: 15, display: "block", marginBottom: 6 }}>
                {themeInfo.fullName}
              </span>
              <span className="pixel-text" style={{ color: C.textDim, fontSize: 12, display: "block", marginBottom: 4 }}>
                Win 2 of 3 lanes over {ROUND_COUNT} rounds to win.
                Each round you get SP to place cards. Cards in lanes score their power.
              </span>
              <span className="pixel-text" style={{ color: C.textDim, fontSize: 12 }}>
                Your deck: {player.deck.length} cards · Opponent scales with your DCC level.
              </span>
            </div>
            ...
            {pendingCardUpgrades > 0 && (
              <div style={{ backgroundColor: "#001A00", border: `2px solid ${C.green}`, padding: "8px 12px" }}>
                <span className="pixel-text" style={{ color: C.green, fontSize: 14 }}>
                  ★ {pendingCardUpgrades} card upgrade{pendingCardUpgrades > 1 ? "s" : ""} available — go to Cards tab
                </span>
              </div>
            )}
