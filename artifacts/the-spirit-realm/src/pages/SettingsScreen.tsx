...
function Section({ title, color, children }: { title: string; color?: string; children: React.ReactNode }) {
  return (
    <div style={{ backgroundColor: C.bg2, border: `2px solid ${color ?? "#222"}` }}>
...
export default function SettingsScreen() {
  ...
  function handleExportSave() {
    try {
      ...
      if (!raw) { flash("No save data found."); return; }
      const blob = new Blob([raw], { type: "application/json" });
      const url  = URL.createObjectURL(blob);
      const a    = document.createElement("a");
      a.href     = url;
      a.download = `spirit-realm-save-${new Date().toISOString().slice(0, 10)}.json`;
      a.click();
  ...
  function handleImportSave(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = (ev) => {
      try {
        ...
        flash("Save imported! Reload the page to apply.");
      } catch { flash("Invalid save file."); }
    };
        ...
        {tab === "combat" && (
          <>
            <Section title="COMBAT — HOW IT WORKS" color={C.redBright}>
              ...
              <InfoRow label="[CONFUSE]"       desc="Confuse effect: causes the enemy to attack itself instead of you for one round. Applied by Confuse weapons." />
              <InfoRow label="[LIGHTNING]"     desc="Lightning effect: chains to all other enemies dealing bonus damage. Great against groups." />
              ...
              <InfoRow label="Resistance"      desc="Each enemy is also resistant to one damage type (opposite of their weakness). Attacking their resistant type deals −4 damage." />
          ...
          <>
            <Section title="SAVE / LOAD — CROSS-DEVICE" color={C.green}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                <PixelButton color="#001A00" textColor={C.green} onClick={handleExportSave}>
                  ↓ EXPORT SAVE
                </PixelButton>
                <PixelButton color="#0A0A18" textColor="#80A0FF" onClick={() => importRef.current?.click()}>
                  ↑ IMPORT SAVE
                </PixelButton>
                <input
                  ref={importRef}
              ...
              <span className="pixel-text" style={{ color: C.textDim, fontSize: 11 }}>
                After importing, reload the page to apply the new save.
              </span>
            </Section>
            ...
            <Section title="IMAGE REPLACEMENT GUIDE" color="#5080C0">
              ...
              <span className="pixel-text" style={{ color: "#80B0FF", fontSize: 13 }}>
                On GitHub (web):
              </span>
              ...
              <span className="pixel-text" style={{ color: "#80B0FF", fontSize: 13 }}>
                On PC (downloaded game):
              </span>
              <div style={{ display: "flex", flexDirection: "column", gap: 4, paddingLeft: 8, borderLeft: "2px solid #50C0FF55" }}>
                <span className="pixel-text" style={{ color: "#80D0FF", fontSize: 12 }}>1. Open File Explorer and navigate to the game folder</span>
                <span className="pixel-text" style={{ color: "#80D0FF", fontSize: 12 }}>2. Go into: artifacts → the-spirit-realm → public → art</span>
                <span className="pixel-text" style={{ color: "#80D0FF", fontSize: 12 }}>3. Drop your PNG file in (use the same filename as the one you want to replace)</span>
                <span className="pixel-text" style={{ color: "#80D0FF", fontSize: 12 }}>4. For JWC combat sprites, put them in the jwc subfolder</span>
                <span className="pixel-text" style={{ color: "#80D0FF", fontSize: 12 }}>5. Reload the game — new image shows instantly, no code changes needed</span>
              </div>
              <span className="pixel-text" style={{ color: C.textDim, fontSize: 11 }}>
                Sprite maps: ENEMY_SPRITES (combat), CHARACTER_SPRITES (player/NPCs), CARD_SPRITES (Black Rose cards). Images render at pixel-perfect scale; 64×64 or 96×96 PNGs work best.
              </span>
            </Section>
            ...
            <Section title="DEBUG TOOLS" color="#8060C0">
              ...
              <div style={{ borderTop: `1px solid #60C8FF33`, paddingTop: 8 }}>
                <span className="pixel-text" style={{ color: "#60C8FF", fontSize: 13, display: "block", marginBottom: 6 }}>
                  ⟳ REBIRTH DEBUG — count: {player.rebirthCount}
                </span>
              ...
              <div style={{ borderTop: `1px solid #FF606033`, paddingTop: 8 }}>
                <span className="pixel-text" style={{ color: C.redBright, fontSize: 13, display: "block", marginBottom: 6 }}>
                  ⚔ FIGHT SEQUENCE — Group → Elite → Boss (current biome)
                </span>
