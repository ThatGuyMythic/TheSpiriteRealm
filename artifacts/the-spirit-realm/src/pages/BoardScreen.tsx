...
function ForgeOverlay({ onClose, inline }: { onClose: () => void; inline?: boolean }) {
  ...
  function doMerge() {
    ...
    const r = mergePartIntoWeapon(partSel, gearSel);
    flash(r.msg);
    if (r.ok) { setPartSel(null); setGearSel(null); }
  }
            ...
            <div>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 4 }}>
                {(() => {
                  ...
                  return Array.from(groups.entries()).map(([name, group]) => {
                    const rep = group[0];
                    const isSel = partSel === rep.id || group.some(p => p.id === partSel);
                    return (
                      <PixelButton key={name} small
                        color={isSel ? C.yellow : C.bg3}
                        textColor={isSel ? "#000" : C.textDim}
                        onClick={() => setPartSel(isSel ? null : rep.id)}>
              ...
              {partSel && (
                <>
                  ...
                  {gearSel && (() => {
                    const part = parts.find(p => p.id === partSel) as EnemyPart | undefined;
...
export default function BoardScreen() {
  ...
  function animateTo(startIdx: number, endIdx: number) {
    const start = tileCoord(startIdx);
    const end   = tileCoord(endIdx);
    let t = 0;
    const STEPS = 20;
    const tick = () => {
      t++;
      const p = t / STEPS;
      setAnimPos({ row: start.row + (end.row - start.row) * p, col: start.col + (end.col - start.col) * p });
      if (t < STEPS) animRef.current = requestAnimationFrame(tick);
      else           setAnimPos(end);
    };
    ...
    <div style={{
      ...
      <div style={isDesktop ? {
        ...
        <div style={{
          ...
          <div style={{
            position: "absolute",
            left:  curPos.col * TILE_SIZE + TILE_SIZE / 2 - 9,
            top:   curPos.row * TILE_SIZE + TILE_SIZE / 2 - 9,
            width: 18, height: 18,
            backgroundColor: "#4DBBCC",
