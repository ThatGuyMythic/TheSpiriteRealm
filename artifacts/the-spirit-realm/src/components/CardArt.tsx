...
const WHITE_BG_CARDS = new Set(["Bone Walker","Lich Spawn","Fire Imp","Warchief"]);
...
export default function CardArt({ name, size = 40 }: { name: string; size?: number }) {
  ...
  const darkBg  = WHITE_BG_CARDS.has(name);
  return (
    <div style={{
      ...
      {imgSrc
        ? <img src={imgSrc} alt={name} style={{ width:"100%", height:"100%", objectFit:"cover", imageRendering:"pixelated", mixBlendMode: darkBg ? "multiply" : undefined }} />
  ...
  return (
    ...
    >
      ...
      <div style={{ flex:1, overflow:"hidden", position:"relative", backgroundColor: "#060612" }}>
        {imgSrc
          ? <img src={imgSrc} alt={card.name} style={{ width:"100%", height:"100%", objectFit:"cover", imageRendering:"pixelated", mixBlendMode: WHITE_BG_CARDS.has(card.name) ? "multiply" : undefined }} />
