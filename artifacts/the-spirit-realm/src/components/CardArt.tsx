...
export function FullCard({ card, selected, onClick, dimmed, isEnemy }: FullCardProps) {
  const col    = nameColor(card.name);
  const frameW = 60;
  const frameH = 92;
  const artH   = frameH - 22;
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        padding:"2px 3px", gap:2,
      }}>
        <div style={{
          backgroundColor:col, width:11, height:11, flexShrink:0,
          display:"flex", alignItems:"center", justifyContent:"center",
          border:"1px solid #000",
        }}>
          <span className="pixel-text" style={{ color:"#000", fontSize:8, lineHeight:1 }}>{card.cost}</span>
        </div>
        <span className="pixel-text" style={{ color:col, fontSize:7, flex:1, overflow:"hidden", whiteSpace:"nowrap", lineHeight:1 }}>
          {card.name}
        </span>
        <div style={{
          backgroundColor: isEnemy ? C.redBright : C.yellow, minWidth:11, height:11, flexShrink:0,
          display:"flex", alignItems:"center", justifyContent:"center",
          border:"1px solid #000", padding:"0 1px",
        }}>
          <span className="pixel-text" style={{ color:"#000", fontSize:8, lineHeight:1 }}>{card.power}</span>
        </div>
      </div>
      ...
      <div style={{ flex:1, overflow:"hidden", position:"relative", backgroundColor:"#060612" }}>
        ...
        <div style={{
          position:"absolute", bottom: card.text ? 14 : 0, left:0, right:0,
          backgroundColor: col + "18",
          padding:"1px 3px",
        }}>
          <span className="pixel-text" style={{ color: col + "99", fontSize:7 }}>
            {isEnemy ? "ENEMY" : "SPIRIT"}
          </span>
        </div>
        {card.text && (
          <div style={{ position:"absolute", bottom:0, left:0, right:0, padding:"1px 3px", backgroundColor:"#08080Ecc" }}>
            <span className="pixel-text" style={{ color:C.textDim, fontSize:7, lineHeight:1.2 }}>
              {card.text}
            </span>
          </div>
        )}
